...
#     Singleton pattern: uses static methods.
#     """

#     @staticmethod
#     def get_movie_details(db: Session, movie_id: int) -> Optional[dict]:
#         """
#         Retrieve detailed information about a movie by ID.
#         Includes director, genres, cast, and calculated average_rating.
...
#         """
#         movie = MovieRepository.get_by_id(db=db, movie_id=movie_id)
...
#             total_score = sum(rating.score for rating in movie.ratings)
...
#     # ✅ متد جدید برای ایجاد فیلم (POST /api/v1/movies/)
#     @staticmethod
#     def create_movie(
#         db: Session,
#         title: str,
...
#         Returns:
#             Dictionary with created movie details or None if validation fails
#         """
#         # 1. بررسی وجود کارگردان
#         director = db.query(Director).filter(Director.id == director_id).first()
#         if not director:
#             return None  # کارگردان پیدا نشد
        
#         # 2. بررسی وجود تمام ژانرها
#         genres = db.query(Genre).filter(Genre.id.in_(genre_ids)).all()
#         if len(genres) != len(genre_ids):
#             return None  # یک یا چند ژانر پیدا نشد
...
#             "cast": cast  # SQLAlchemy از ARRAY پشتیبانی می‌کند
#         }
        
#         new_movie = MovieRepository.create(db=db, movie_data=movie_data)
        
#         # 4. اضافه کردن ژانرها به فیلم (رابطه many-to-many)
#         new_movie.genres = genres
#         db.commit()
#         db.refresh(new_movie)
        
#         # 5. برگرداندن پاسخ به فرمت مورد نیاز
#         return {
#             "id": new_movie.id,
#             "title": new_movie.title,
#             "release_year": new_movie.release_year,
#             "director": {
#                 "id": new_movie.director.id,
#                 "name": new_movie.director.name
#             },
#             "genres": [genre.name for genre in new_movie.genres],
#             "cast": new_movie.cast,
#             "average_rating": None,  # فیلم جدید هنوز امتیازی نداره
#             "ratings_count": 0
#         }
